...
def generate_charts(parsed_log: dict, metrics: dict, output_dir: str) -> list:
    ...
    generate_chart_data(parsed_log, metrics, output_dir)

    # Filter existing files
    return [c for c in charts if os.path.exists(c)]


def generate_chart_data(parsed_log: dict, metrics: dict, output_dir: str) -> str:
    """Generate structured chart data for frontend interaction."""
    os.makedirs(output_dir, exist_ok=True)
    raw = parsed_log.get("raw_sample", {})
    charts = []

    attitude = _attitude_chart_spec(raw)
    if attitude:
        charts.append(attitude)

    vibration = _vibration_chart_spec(raw)
    if vibration:
        charts.append(vibration)

    fft = _fft_chart_spec(raw, parsed_log)
    if fft:
        charts.append(fft)

    motors = _motors_chart_spec(raw)
    if motors:
        charts.append(motors)

    battery = _battery_chart_spec(raw)
    if battery:
        charts.append(battery)

    payload = {"version": 1, "charts": charts, "metrics": metrics}
...
def _series(name, color, values, unit=""):
    points = [[idx, float(value)] for idx, value in enumerate(np.asarray(values, dtype=float).tolist())]
    return {"name": name, "color": color, "unit": unit, "points": points}


def _attitude_chart_spec(raw: dict):
    ...
    return {
        ...
        "yLabel": "Degree",
        "series": [
            _series("Roll", "#2563eb", roll, "°"),
            _series("Pitch", "#f59e0b", pitch, "°"),
            _series("Yaw", "#ef4444", yaw, "°"),
        ],
    }


def _vibration_chart_spec(raw: dict):
    ...
    return {
        ...
        "yLabel": "m/s²",
        "thresholds": [
            {"value": 8, "label": "正常上限", "color": "#16a34a"},
            {"value": 15, "label": "注意", "color": "#f59e0b"},
            {"value": 30, "label": "严重", "color": "#dc2626"},
        ],
        "series": [_series("Vibration", "#2563eb", mag, "m/s²")],
    }


def _fft_chart_spec(raw: dict, parsed_log: dict):
    sc = raw.get("sensor_combined", {}).get("data_subset", {})
    accel_data = _get_chart_data(sc, "accelerometer_m_s2")
    if accel_data:
        accel = [np.asarray(accel_data[k], dtype=float) for k in sorted(accel_data.keys())]
    ...
    sample_rate = len(mag) / duration
    fft_vals = np.abs(np.fft.rfft(mag - np.mean(mag)))
    fft_freqs = np.fft.rfftfreq(len(mag), d=1.0 / sample_rate)
    points = [[float(freq), float(value)] for freq, value in zip(fft_freqs, fft_vals) if freq <= 200]
    return {
        "id": "fft",
        "title": "振动频谱",
        "xLabel": "Hz",
        "yLabel": "Magnitude",
        "series": [{"name": name, "color": "#2563eb", "unit": "", "points": points}],
    }


def _motors_chart_spec(raw: dict):
    ctrl = raw.get("actuator_motors", {}).get("data_subset", {})
    control_keys = sorted([key for key in ctrl.keys() if key.startswith("control[")])
    if not control_keys:
        return None
    palette = ["#2563eb", "#f59e0b", "#ef4444", "#7c3aed", "#0891b2", "#db2777", "#65a30d", "#475569"]
    return {
        "id": "motors",
        "title": "电机输出",
        "xLabel": "Sample",
        "yLabel": "Output",
        "thresholds": [{"value": 1, "label": "饱和", "color": "#dc2626"}],
        "series": [
            _series(f"Motor {idx}", palette[idx % len(palette)], np.asarray(ctrl[key], dtype=float), "")
            for idx, key in enumerate(control_keys[:8])
        ],
    }


def _battery_chart_spec(raw: dict):
    battery = raw.get("battery_status", {}).get("data_subset", {})
    volt = battery.get("voltage_v")
    curr = battery.get("current_a_sensors_average") or battery.get("current_a")
    if volt is None:
        return None
    series = [_series("Voltage", "#2563eb", volt, "V")]
    if curr is not None:
        series.append(_series("Current", "#f59e0b", curr, "A"))
    return {
        "id": "battery",
        "title": "电池状态",
        "xLabel": "Sample",
