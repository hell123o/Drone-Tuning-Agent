...
function getServerCwd() {
  ...
  return path.join(process.resourcesPath, 'frontend-standalone');
}
...
function getServerEntry() {
  ...
  return path.join(process.resourcesPath, 'frontend-standalone', 'server.js');
}
...
function ensurePackagedCore() {
  ...
  const targetRuns = path.join(projectRoot, 'runs');
...
async function startServer() {
  ...
  const env = {
    ...
    DRONE_AGENT_BACKEND_ROOT: isPackaged() ? projectRoot : path.resolve(__dirname, '..', 'backend'),
  };
